...
class usersession
{
    ...
    */
    ...
    {
        $username = user::login($user, $pass);//return username
        ...
        $sql = "INSERT INTO `session_table` (`uid`, `token`, `lastactive_time`, `login_time`, `ip`, `user_agent`, `active`)
            VALUES ('$user->id', '$token',now(), now(), '$ip', '$agent','1')";
        if ($conn->query($sql)) {
